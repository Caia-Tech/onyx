...
# Reasonable starting point for Onyx 1B on a single H100 80GB.
# You will likely want to tune these based on your data + stability.

export RUN_NAME="${RUN_NAME:-onyx_1b_h100}"

# Dataset/tokenizer paths are intentionally not set here.
# Set them explicitly, e.g.:
#   export DATA_GLOB=/workspace/data/datamix.jsonl
#   export TOKENIZER_DIR=/workspace/tokenizer

export MODEL_CONFIG="${MODEL_CONFIG:-$PWD/configs/onyx_1b.json}"
...
# tokens_per_step controls gradient accumulation:
...
export WARMUP_RATIO="${WARMUP_RATIO:-0.0002}"
...
# Helps with allocator fragmentation on long runs.
